...
def test_a_registered_email_should_return_a_club(test_club_as_list):
    ...
    """
    email = "test@club.com"
    assert isinstance(get_club_by_email(email, test_club_as_list), Club)
...
def test_a_registered_club_name_should_return_the_matching_club(test_club_as_list):
    """
    Checks that a registered club name returns the club
    """
    club_name = "Test Club"
    assert isinstance(get_club_by_name(club_name, test_club_as_list), Club)
...
def test_an_unregistered_club_name_should_return_none(test_club_as_list):
    ...
    assert get_club_by_email(club_name, test_club_as_list) is None
...
def test_update_club_points(test_club_as_obj, test_new_club_points):  # test à écrire
